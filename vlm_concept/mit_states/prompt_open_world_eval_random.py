...
def get_precomputed_data_root(args):
    return f"./outputs/mit_states/{args.model}_prompt_precompute_features_random/{args.template_src}/num_prompts_{args.num_prompts}"

def get_output_path(args):
    output_name = "{}_prompt_retrieval_model_TP{}_NP{}_open{}_init{}_imgproj{}_bias{}_{}_Lim{}_N{}_TW{}_B{}_LR{}_WD{}_L{}".format(
        args.model,
        args.template_src,
        args.num_prompts,
    ...
    output_path = f"./outputs/mit_states/{args.model}_prompt_open_world_eval_random/{output_name}"
    os.makedirs("./outputs/mit_states", exist_ok=True)
    os.makedirs(f"./outputs/mit_states/{args.model}_prompt_open_world_eval_random", exist_ok=True)
    os.makedirs(output_path, exist_ok=True)
    return output_path, output_name

def get_logger(args):
    os.makedirs("./logs/mit_states", exist_ok=True)
    os.makedirs(f"./logs/mit_states/{args.model}_prompt_open_world_eval_random", exist_ok=True)
    os.makedirs("./logs/mit_states/{}_prompt_open_world_eval_random/{}".format(args.model, args.output_name), exist_ok=True)
    logging.basicConfig(level=logging.INFO, \
            format = "%(asctime)s %(levelname)s: %(message)s", \
            datefmt = "%m/%d %H:%M:%S %p", \
            filename = "./logs/mit_states/{}_prompt_open_world_eval_random/{}/{}.log".format(args.model, args.output_name, args.output_name), \
            filemode = "w"
    )

...
def parseArguments(): 
    ...
    # Necessary variables
    parser.add_argument("--ablation_model", type=int, choices=[0,1,2],
                        help="0 for image emb + linear cls. 1 for image emb + learned proj. + linear cls. 2 for image emb + random proj. + linear cls.")
    parser.add_argument("--feature", type=str, default="image",
                        help="['image', 'primitive', 'pair', 'all', 'gt_primitive']")
...
def get_output_path(args):
    output_name = "{}_image_ablation_{}_open{}_init{}_imgproj{}_bias{}_{}_Lim{}_N{}_TW{}_B{}_LR{}_WD{}_L{}_O{}".format(
        args.model,
        args.ablation_model,
    ...
    output_path = f"./outputs/mit_states/{args.model}_image_ablation/{output_name}"
    os.makedirs("./outputs/mit_states", exist_ok=True)
    os.makedirs(f"./outputs/mit_states/{args.model}_image_ablation", exist_ok=True)
    os.makedirs(output_path, exist_ok=True)
    return output_path, output_name

def get_logger(args):
    os.makedirs("./logs/mit_states", exist_ok=True)
    os.makedirs(f"./logs/mit_states/{args.model}_image_ablation", exist_ok=True)
    os.makedirs("./logs/mit_states/{}_image_ablation/{}".format(args.model, args.output_name), exist_ok=True)
    logging.basicConfig(level=logging.INFO, \
            format = "%(asctime)s %(levelname)s: %(message)s", \
            datefmt = "%m/%d %H:%M:%S %p", \
            filename = "./logs/mit_states/{}_image_ablation/{}/{}.log".format(args.model, args.output_name, args.output_name), \
            filemode = "w"
    )
...
class Precomputed_MITStatesDataset(Dataset):
    def __init__(self, split, feature, data, args, is_limit=True):
        ...
        if feature == "primitive":
            ...
            image_embs_train = np.load(os.path.join(args.precomputed_data_root, "image_features_train.npy"))
            image_embs_valid = np.load(os.path.join(args.precomputed_data_root, "image_features_valid.npy"))
            image_embs_test = np.load(os.path.join(args.precomputed_data_root, "image_features_test.npy"))
...
class AblationModel(nn.Module):
    def __init__(self, data, image_dim, limit_pairs, open_world_pairs, args):
        ...
        self.num_primitive_concepts = 360  # 115 attributes and 245 objects
        ...
        if args.ablation_model == 0:
            # image embeddings + linear cls
            pass
        elif args.ablation_model == 1:
            # image embeddings + learned projection layer + linear cls
            self.ablation_image_projection = nn.Linear(512, self.num_primitive_concepts, bias=args.is_bias)
            self.gelu = nn.GELU()
        elif args.ablation_model == 2:
            # image embeddings + random projection layer + linear cls
            self.ablation_image_projection = nn.Linear(512, self.num_primitive_concepts, bias=args.is_bias)
            self.ablation_image_projection.weight.requires_grad = False
            self.gelu = nn.GELU()
        ...
        if self.args.is_image_projection:
            ...
            if self.args.ablation_model == 0:
                self.image_projection = nn.Linear(512, self.output_dim, bias=args.is_bias)
            elif self.args.ablation_model in [1,2]:
                self.image_projection = nn.Linear(self.num_primitive_concepts, self.output_dim, bias=args.is_bias)
    ...
    def forward(self, image_embs, pair_labels=None, is_train=True):
        ...
        if self.args.ablation_model in [1,2]:
            image_embs = self.ablation_image_projection(image_embs.float())

        if self.args.is_image_projection:
            if self.args.ablation_model in [1,2]:
                image_embs = self.gelu(image_embs)
...
def main(args):
    ...
    model = AblationModel(data, train_dataset.image_dim, limit_pairs, open_world_pairs, args)
    model.to(device)
    if args.emb_init:

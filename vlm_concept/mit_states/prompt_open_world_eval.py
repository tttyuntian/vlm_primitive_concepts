...
from ..mit_states.tmn_mit_states_data import MITStatesDataset, get_precomputed_features, normalize, get_gt_primitives
...
def parseArguments(): 
    ...
    parser.add_argument("--template_id", type=str, help="clip up to 6, and compdl up to 10.")
    parser.add_argument("--is_combined", action="store_true")
    parser.add_argument("--split", type=str, required="train")
    ...
    # I/O parameters
    precomputed_data_root = get_precomputed_data_root(args)
    output_path, output_name = get_output_path(args)
    parser.add_argument("--precomputed_data_root", type=str, default=precomputed_data_root)
    parser.add_argument("--output_path", type=str, default=output_path)
    parser.add_argument("--output_name", type=str, default=output_name)
    args = parser.parse_args()
    return args

def get_precomputed_data_root(args):
    if args.is_combined:
        return f"./outputs/mit_states/{args.model}_prompt_precompute_features/{args.template_src}/combined"
    else:
        return f"./outputs/mit_states/{args.model}_prompt_precompute_features/{args.template_src}/template_{args.template_id}"

def get_output_path(args):
    output_name = "{}_prompt_retrieval_model_TP{}_TID{}_open{}_init{}_imgproj{}_bias{}_{}_Lim{}_N{}_TW{}_B{}_LR{}_WD{}_L{}".format(
        args.model,
        args.template_src,
        "all" if args.is_combined else args.template_id,
    ...
    output_path = f"./outputs/mit_states/{args.model}_prompt_open_world_eval/{output_name}"
    os.makedirs("./outputs/mit_states", exist_ok=True)
    os.makedirs(f"./outputs/mit_states/{args.model}_prompt_open_world_eval", exist_ok=True)
    os.makedirs(output_path, exist_ok=True)
    return output_path, output_name

def get_logger(args):
    os.makedirs("./logs/mit_states", exist_ok=True)
    os.makedirs(f"./logs/mit_states/{args.model}_prompt_open_world_eval", exist_ok=True)
    os.makedirs("./logs/mit_states/{}_prompt_open_world_eval/{}".format(args.model, args.output_name), exist_ok=True)
    logging.basicConfig(level=logging.INFO, \
            format = "%(asctime)s %(levelname)s: %(message)s", \
            datefmt = "%m/%d %H:%M:%S %p", \
            filename = "./logs/mit_states/{}_prompt_open_world_eval/{}/{}.log".format(args.model, args.output_name, args.output_name), \
            filemode = "w"
    )
...
class Precomputed_MITStatesDataset(Dataset):
    def __init__(self, split, feature, data, args, is_limit=True):
        ...
        if feature == "primitive":
            ...
            if is_limit:
                print(image_embs_tuple[0].shape)
...
def main(args):
    ...
    ckpt_name = "clip_prompt_retrieval_model_TP{}_TID{}_open{}_init{}_imgproj{}_bias{}_{}_Lim{}_N{}_TW{}_B{}_LR{}_WD{}_L{}".format(
        args.template_src,
        args.template_id,
        0, # we take the ckpt optimized on closed-world setting
        1 if args.emb_init else 0,
        1 if args.is_image_projection else 0,
        1 if args.is_bias else 0,
        args.feature,
    ...
    logger.info("Preparing primitive concept activations")
    ...
    with open(f"{args.output_path}/{args.split}_metrics.json", "w") as f:
        json.dump(overall_metrics, f)

    logger.info(f"{args.split} split metrics saved to {args.output_path}")
    logger.info("Done!")

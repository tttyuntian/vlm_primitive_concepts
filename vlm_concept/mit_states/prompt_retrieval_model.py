...
from .tmn_mit_states_data import MITStatesDataset, normalize, get_gt_primitives
...
def parseArguments(): 
    ...
    parser.add_argument("--template_id", type=int, help="clip up to 6, and compdl up to 10.")
    parser.add_argument("--is_combined", action="store_true")
    parser.add_argument("--feature", type=str, required=True, 
                        choices="['primitive', 'pair', 'all', 'gt_primitive']")
...
def get_output_path(args):
    ...
    output_path = f"./outputs/mit_states/{args.model}_prompt_retrieval_model/{output_name}"
    os.makedirs("./outputs/mit_states", exist_ok=True)
    os.makedirs(f"./outputs/mit_states/{args.model}_prompt_retrieval_model", exist_ok=True)
    os.makedirs(output_path, exist_ok=True)
    return output_path, output_name

def get_logger(args):
    os.makedirs("./logs/mit_states", exist_ok=True)
    os.makedirs(f"./logs/mit_states/{args.model}_prompt_retrieval_model", exist_ok=True)
    os.makedirs("./logs/mit_states/{}_prompt_retrieval_model/{}".format(args.model, args.output_name), exist_ok=True)
    logging.basicConfig(level=logging.INFO, \
            format = "%(asctime)s %(levelname)s: %(message)s", \
            datefmt = "%m/%d %H:%M:%S %p", \
            filename = "./logs/mit_states/{}_prompt_retrieval_model/{}/{}.log".format(args.model, args.output_name, args.output_name), \
            filemode = "w"
    )
...
class Precomputed_MITStatesDataset(Dataset):
    def __init__(self, split, feature, data, args, is_limit=True):
        ...
        if feature == "primitive":
            ...
            if (is_limit) and (args.model == "clip"):
